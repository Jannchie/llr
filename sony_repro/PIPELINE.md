...
**`YGamma`(RVA 0x36f030)只动平面0(Y),是第二条色调曲线 + 黑白场。**
实测平面1/平面2 的改动比例 **0.0000%**。

```
lut   = calib + 0x318fc                        # uint16 表,按 Y 索引
black = -((bl + (bl>>31 & 3)) >> 2) * k1 * k2   # bl = settings[0x2ac]
scale = K / ((0x200 - settings[0x2b0]) - black_int)
y = clamp(((max(0, lut[Y]) - black) * scale - pivot) * contrast + pivot, 0, 16383)
```
`pivot = (short)calib[0x91962]`,`contrast = (float)calib[0x91964]`。
实测这一段把 Y 的均值从 3383.6 抬到 3564.0,**不是恒等,复刻时不能略过**。
