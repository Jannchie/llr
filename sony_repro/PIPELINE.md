...
## 5. LinearMatrix16(结构已解出,但**系数来源是错的**,见 §11)
...
> **注意:「与内存中的数组最大差 0.0」这条已被推翻。** 2026-07-26 重新抓引擎的
> 1024 项数组(`tools/matrix_table.py`)再比,十种外观平均 |差| 都在 0.09~0.22。
> 展开规则本身没问题,错的是喂进去的节点系数 —— 见 §11。
...
> **「离线解出的表与引擎内存逐位一致」这条也已被推翻**(同上)。位流解读经
> 反编译逐条核对**是对的**(§11.2),但 tag `0x780f` 的 276 字节**不是**引擎
> 真正解包的那 276 字节。我们解出的节点 0 恰好等于 tag `0x7800` 的普通 3x3,
> 而引擎的节点 0 与之不同。
...
## 11. 误差的真正位置:LinearMatrix16 的 1024 张矩阵**不对**

一段一段切下来(全部拿引擎自己的前后两份逐像素比):

| 环节 | 结果 |
|---|---|
| 色调曲线(喂引擎自己的矩阵出口) | **最大差 1/16383,色度比 1.0001,色相移 0.00 度** ✅ |
| YCC 段(§7.6) | **Y 逐位相同,色度差 ≤1** ✅ |
| **矩阵**(喂引擎自己的入口像素) | R-G 比值 1.0140 ✅ / **B-G 比值 0.6817** ❌ |

于是「我们的矩阵+曲线」对「引擎的 MainGamma 出口」:色度 **0.9047**、色相 **-7.57 度**。
误差全在矩阵这一步,**与 YCC 段无关,也不缺任何阶段**。

**引擎展开好的 1024 张矩阵已经抓下来了**(`tools/matrix_table.py`):在 kernel
`0x37f8e0` 的**参数3 + 0x228**,条目步长 **72 字节**(asm 是 `[param + rax*8 + 0x228]`
而 `rax = idx*9`),前 9 个 float32 有效、后 9 个是填充。每行和为 1 的比例 100.0%。

与我们 `unpack_param_block` + `expand` 出来的表比(`tools/matrix_table_cmp.py`):

```
ST 0.155  VV 0.116  NT 0.214  PT 0.155  FL 0.148  VV2 0.094  IN 0.225  SH 0.177
```
**十种外观全都对不上**(平均 |差| 0.09~0.22,而系数本身量级才 0.15),
任何环形位移也救不了。所以错的是 **276 字节 `0x780f` 位流的解读**
(`sr2.py` 的 `unpack_param_block` / `_decode14`),不是色相索引、不是展开。

### 11.2 但位流解读**是对的** —— 错的是喂进去的那 276 字节

把解包函数 `+0x179390` 整个反编译出来逐条核对(`tools/pyg_unpack.py`),
`sr2.py` 的实现与原文**完全一致**:
...
所以问题不在解包,**在于我们喂给它的不是引擎喂给它的那 276 字节**。
引擎从 `param_2 + 0xae` 取,而我们从 DataIFD 的 tag `0x780f` 取。

决定性线索:DataIFD 里 tag **`0x7800`** 是个普通 3x3,
`[1144 -219 99 -211 1389 -154 24 -144 ...] / 1024`
= `[1.117 -0.214 0.097 | -0.206 1.356 -0.150 | 0.023 -0.141 ...]`,
**与我们解出的节点 0 一模一样**;而引擎的节点 0 是
`[0.973 -0.220 0.247 | -0.241 1.178 0.063 | 0.194 -0.102 0.907]`。
我们读到的是一份**相关但不同**的标定。

**最可能的解释:矩阵和色度参数(§7.2)一样,是「基准 + 光源增量按白平衡插值」。**
0x7842 有 `0x7843..0x7846` 四组增量,矩阵多半也有一组,只是不在同一个 DataIFD 里
(该 IFD 的 `0x7800..0x7850` 已列全,没有 0x780f 的兄弟)。

**下一步:回溯 `param_2` 是谁填的**,找出那 276 字节的真正来源。
现在有了逐位可比的靶子(`tools/matrix_table.py` 抓引擎的表和系数,
`tools/coeff_cmp.py` 逐个对),不用再猜。

### 11.1 三条试过不成立的路

1. **按色相分桶逐桶解 3x3**:一个色相段里 `(R-G, B-G)` 近乎共线,两个系数分不开,
   末几段会解出 `-0.55 / +0.55` 这种互相抵消的病态值。**改用全局最小二乘**
   (索引已知则输出对 96 个未知数是线性的),残差中位 0.00006 —— 模型结构本身没问题。
2. **6 行系数换摆法**(720 种全试):最好的一种只把平均 |差| 从 0.00145 降到 0.00095,
   而且前几名互相矛盾(`m12 m02 m01 m10 m20 m21`),是在拟合噪声。
   `reshape(16,6).T` 等排布也都更差。
3. **色相索引整体偏移**:逐像素反查最贴合的段,得到统一偏移 **+194**(残差 0.00864
   -> 0.00403),看着很像答案。但真值表一比就知道**节点矩阵本身就不对**,
   这个 +194 是矩阵错误在索引上的投影,不是独立的 bug。
...
| `matrix_solve.py` / `hue_index_check.py` | 反解节点、验色相索引(结论见 §11.1) |
