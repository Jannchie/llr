...
候选参数:DataIFD 的 `0x7842` / `0x7844` / `0x7845`(各 int16[8])。
BW 外观下这些值**全是 0**,且后四个值 VV=1026,778,1096 / NT=770,866,922 /
FL=664,658,660 / IN=436,702,402,与各外观实际饱和度高低一致。
格式未解(值 /1024 都 <1,而实测需要 >1)。下一步应反汇编 `ZcTaskSSCS`。

## 7. 3D-LUT:已定位、已抓到数据,但**默认路径不执行**
