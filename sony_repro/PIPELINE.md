...
### 7.6.1 光源权重是**变的**,这是目前最大的已知缺口

`FUN_14036dce0`(反编译)给出的公式是:
...
(另有捷径:`calib+0x1c != 0` 时直接取 `+0xddc` 的成品;`param_4 == 0xb` 时只加第 0 组。)

实测三张:DSC03015 = `(1024,0,0,0)`,DSC02990 = `(·,0,0,0)`,
**DSC02961 ≈ `(340,684,0,0)`** —— 第 0 组增量全零所以 w0 不可观测,但和为 1024 说得通,
看着像**按色温在相邻光源之间插值**。

代价已量化:在权重不同的那张图上,只用基准会让色相偏 **4.54 度**;
把引擎实测的八个值塞进同一条管线,色相立刻回到 **0.03 度**(色度 1.062 -> 1.039)。
**所以色相残差就是这一处,不是 RGB2YCC 本身。** 但色度那 40% 的缺口更显眼,
先接上是划算的。
