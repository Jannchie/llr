...
> ⚠️ 「SSCS 饱和」这个说法后来查明是**误归因** —— 那个色度缺口来自 RGB2YCC,
> 已解完(`sony/chroma.py`,逐像素验过)。详见下面那一节。
...
### 曾经的「SSCS 饱和段」——已经不是一件事了

> **本节整段是历史记录。** 当时把三件事捆成了一个名字,后来发现它们互不相干,
> 其中两件已经解完。留着是因为「候选参数猜错了格式」那一段值得记。

当时的观测:与机内 JPEG 逐像素比(9 张 DRO 关、微调为零的 VV2),**亮度已完全对上**,
缺口只在色度 —— 增益随色相变化 0.64~1.45,伴随几度色相旋转,单张之间差异极大
(DSC03015 是 1.23,DSC03022 只有 0.88),**不能当成常数增益**。

**① 那个色度缺口:已解完,是 RGB2YCC。** 不是 SSCS。见 `sony/chroma.py`:增益按
`u2`/`v2` 的**符号**分支给出随色相变化的增益,交叉项给出色相旋转 —— 正是当时量到的
两个效应。逐像素对引擎验过:Y 逐位相同,Cb/Cr 在 14 位内差 ≤1。

**② 候选参数 `0x7842` / `0x7844` / `0x7845`:格式已解。** 当时卡在「值 /1024 都
小于 1,而实测需要大于 1」。真正的关系不是「除以 1024」,而是(`0x14036dce0`):

```
out[j] = 0x7842[j] + (0x7843[j]*w0 + 0x7844[j]*w1 + 0x7845[j]*w2 + 0x7846[j]*w3) >> 10
w = 0x7848[0..3]          # 四个光源权重,1024 = 1.0
```

即 `0x7842` 是**基底**,`0x7843..0x7846` 是四个**光源增量**,按权重混。当时把增量
当成了绝对值,自然对不上量纲。`calib+0x1c != 0` 时改走 `0x7841`(成品值,直接用)。
标签落点用 `tools/tag_map.py` 可以整张打出来。

**③ `ZcTaskSSCS` 本身:算法已解出(PIPELINE §7.4,RVA 0x3860f0),但没实现。**
它是**高光去饱和**,和上面那个色度缺口没有关系。没实现的原因是它在现有语料上
**触发率 0.00%** —— 整幅最大值只有 3043,而 lo=2380 / hi=7934。
**唯一还开着的问题是这个量程从哪来**,线索在上游 ITP(§7.12)。
`tools/itp_probe.py` 已经读出 ITP 输入侧的归一化是 `scale = coeff/128`,
实测 `coeff=[3302,1056,1056,1700]` —— 白平衡量级,是噪声域归一化,
**不是**写回时的域(写回是 `clamp(bufD+buf1, 0, 16383)`)。所以量程仍未定位,
下一步要抓的是 ITP 出口 / SSCS 入口的整幅平面,不是继续读反汇编。

**BW 与 Sepia 整条回落 DCP:** 它们的色彩矩阵是**单位阵**,去色发生在 YCC 段。
(现在 `chroma.py` 已解出这一段:它们八个色度值全为 0 → Cb=Cr=0 → R=G=B 自然成立。)
