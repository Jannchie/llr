...
用法(Windows 的 python):
    python spica_gaincfg.py <ARW> [--secs 120]
输出 spica_gaincfg.json
"""
import json
import os
import subprocess
...
def main():
    ...
    for name, off in CURVES.items():
        p = [d.get(off + 4 * k) for k in range(6)]
        print(f"  {name:11} a,b,c,d = {p[0]}, {p[1]}, {p[2]}, {p[3]}"
    ...
    out = os.path.join(SCR, "spica_gaincfg.json")
