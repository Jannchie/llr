r"""**已作废,别用。** 见文末;取而代之的是 extract_tone_family.py。

把逐外观的微调形状,改建成**与外观无关的输出域算子**。
...
**为什么作废。** 这里得到的算子是**拟合**出来的,而后来在 Edit.exe 里直接读到了
引擎自己怎么造这张表(`0x17b8dd` / `0x193230` / `0x1903e0`,见 PIPELINE.md 6.2.3):
三个滑块**相加**成一个 gain,再从 37 条静态曲线里按 gain 插值出唯一一张表。
不是「每个字段一个算子再串联」。

差别不是学术性的:串联模型在「高光+对比度」的组合上差 111/16384,而且同一个
「对比度 +1」在两台机身上量出来不一样 —— 后者根本不是机身差异,是两个文件自带的
Highlights/Shadows 不同,把 gain 的**起点**挪到了曲线族疏密不同的位置。
换成引擎自己的构造之后,这两项残差都归零(51 次渲染逐位相同)。

留着这个文件是因为它测出的那批逐外观形状,是「微调与外观无关」这个结论的独立佐证,
而那个结论后来被代码坐实了。但它产出的 look_tuning.npz 已经不再有人加载。
"""
import os
import sys
