...
STRENGTH_FIXTURE = Path(__file__).parent / "fixtures" / "rawnr_strength.npz"


def test_the_iso_strength_is_a_truncated_blend_on_the_way_out() -> None:
    """The engine's own input plane, its four kernels' output, and what it
    wrote back, on two frames twelve stops of ISO apart (fl_test ISO 1250 and
    a7v_donor ISO 100, from rawnr_strength_probe.py). The write-back is
    trunc(s*filt + (1-s)*in) in float32 with s = iso_strength(ISO), exactly;
    full strength is not what the engine wrote, on either frame."""
    z = np.load(STRENGTH_FIXTURE)
    for tag in ("fl", "a7v"):
        s = iso_strength(float(z[f"{tag}_iso"]))
        src = z[f"{tag}_in"].astype(np.float32)
        want = z[f"{tag}_out"].astype(np.float32)
        np.testing.assert_array_equal(apply_strength(z[f"{tag}_filt"], src, s), want)
        assert not np.array_equal(apply_strength(z[f"{tag}_filt"], src, 1.0), want)


def test_the_strength_blend_keeps_the_engines_float32_order() -> None:
    """`s*f + (1-s)*in`, not `in + s*(f-in)`: the two differ in the last bit and
    the last bit decides the truncation. The fixture pins the order; this
    checks the helper does not silently re-associate."""
    z = np.load(STRENGTH_FIXTURE)
    f = z["fl_filt"].astype(np.float32)
    src = z["fl_in"].astype(np.float32)
    s = np.float32(iso_strength(1250.0))
    other = np.trunc(src + s * (f - src))
